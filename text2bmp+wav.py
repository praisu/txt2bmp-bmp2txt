...
def gen_wav(_path, _data_encoded):
    _size1 = small_hex(len(_data_encoded) + 44)
    _size2 = small_hex(len(_data_encoded))
    _wav_head=f'5249,4646,{_size1[:4]},{_size1[4:]},5741,5645,666D,7420,' \
            f'1000,0000,0100,0200,401F,0000,007D,0000,' \
            f'0400,1000,6461,7461,{_size2[:4]},{_size2[4:]}'.split(',')
    with open(f'{_path}.wav', 'ab+') as _f:
        for _k in _wav_head:
            if _k != '':
                _k = int(_k, 16)
                _bk = _k.to_bytes(2, 'big')
                _f.write(_bk)
        _f.write(_data_encoded)
    print('WAV done!')
